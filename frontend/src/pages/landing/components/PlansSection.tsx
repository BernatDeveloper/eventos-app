...
export const PlansSection = () => {
    ...
    return (
        <section className="py-20 px-6 bg-[var(--background-secondary-color)]">
            <h2 className="text-4xl font-semibold text-center mb-12">{t('plans.title')}</h2>
...
const PlanCard = ({ title, description, features, isSelected = false }: PlanCardProps) => (
    <div className={`flex-1 p-8 rounded-[var(--border-radius-large)] shadow-[var(--box-shadow-${isSelected ? 'medium' : 'heavy'})] text-center ${isSelected ? 'bg-[var(--primary-color)]' : 'bg-[var(--background-tertiary-color)]'} transition ${isSelected ? 'text-[var(--text-on-dark-primary)]' : 'text-[var(--text-primary-color)]'}`}>
